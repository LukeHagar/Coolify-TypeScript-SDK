 ...
 * Delete env by UUID.
 */
export async function applicationsDeleteEnv(
  client: CoolifyCore,
  request: operations.DeleteEnvByApplicationUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.DeleteEnvByApplicationUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteEnvByApplicationUuidRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    env_uuid: encodeSimple("env_uuid", payload.env_uuid, {
      explode: false,
  ...
  const path = pathToFunc("/applications/{uuid}/envs/{env_uuid}")(pathParams);
  ...
  >(
    M.json(
      200,
      operations.DeleteEnvByApplicationUuidResponseBody$inboundSchema,
    ),
    M.jsonErr(
