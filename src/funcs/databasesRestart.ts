 ...
 */
export async function databasesRestart(
  client: CoolifyCore,
  request: operations.RestartDatabaseByUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.RestartDatabaseByUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.RestartDatabaseByUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/databases/{uuid}/restart")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.RestartDatabaseByUuidResponseBody$inboundSchema),
    M.jsonErr(
