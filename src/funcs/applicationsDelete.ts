...
export async function applicationsDelete(
  ...
  Result<
    operations.DeleteApplicationByUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteApplicationByUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/applications/{uuid}")(pathParams);

  const query = encodeFormQuery({
    "delete_configurations": payload.delete_configurations,
    "delete_connected_networks": payload.delete_connected_networks,
    "delete_volumes": payload.delete_volumes,
    "docker_cleanup": payload.docker_cleanup,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.DeleteApplicationByUuidResponseBody$inboundSchema),
    M.jsonErr(
