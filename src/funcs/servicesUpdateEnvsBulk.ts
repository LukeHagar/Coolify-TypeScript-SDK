 ...
 */
export async function servicesUpdateEnvsBulk(
  client: CoolifyCore,
  request: operations.UpdateEnvsByServiceUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.UpdateEnvsByServiceUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateEnvsByServiceUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/services/{uuid}/envs/bulk")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, operations.UpdateEnvsByServiceUuidResponseBody$inboundSchema),
    M.jsonErr(
