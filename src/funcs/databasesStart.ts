 ...
 */
export async function databasesStart(
  client: CoolifyCore,
  request: operations.StartDatabaseByUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.StartDatabaseByUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.StartDatabaseByUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/databases/{uuid}/start")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.StartDatabaseByUuidResponseBody$inboundSchema),
    M.jsonErr(
