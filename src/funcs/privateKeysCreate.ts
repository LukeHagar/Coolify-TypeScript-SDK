 ...
 */
export async function privateKeysCreate(
  client: CoolifyCore,
  request: operations.CreatePrivateKeyRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.CreatePrivateKeyResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreatePrivateKeyRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/security/keys")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, operations.CreatePrivateKeyResponseBody$inboundSchema),
    M.jsonErr(
