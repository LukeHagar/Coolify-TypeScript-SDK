 ...
 */
export async function applicationsExecuteCommand(
  client: CoolifyCore,
  request: operations.ExecuteCommandApplicationRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.ExecuteCommandApplicationResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ExecuteCommandApplicationRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/applications/{uuid}/execute")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.ExecuteCommandApplicationResponseBody$inboundSchema),
    M.jsonErr(
