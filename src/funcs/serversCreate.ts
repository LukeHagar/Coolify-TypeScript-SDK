 ...
 * Create Server.
 */
export async function serversCreate(
  client: CoolifyCore,
  request: operations.CreateServerRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.CreateServerResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateServerRequestBody$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, operations.CreateServerResponseBody$inboundSchema),
    M.jsonErr(
