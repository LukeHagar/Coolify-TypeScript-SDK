 ...
 */
export async function servicesCreateEnv(
  client: CoolifyCore,
  request: operations.CreateEnvByServiceUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.CreateEnvByServiceUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateEnvByServiceUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/services/{uuid}/envs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, operations.CreateEnvByServiceUuidResponseBody$inboundSchema),
    M.jsonErr(
