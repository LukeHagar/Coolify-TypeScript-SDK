 ...
 */
export async function serversDelete(
  client: CoolifyCore,
  request: operations.DeleteServerByUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.DeleteServerByUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteServerByUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/servers/{uuid}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.DeleteServerByUuidResponseBody$inboundSchema),
    M.jsonErr(
