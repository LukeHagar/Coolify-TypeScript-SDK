 ...
 * Update Server.
 */
export async function serversUpdate(
  client: CoolifyCore,
  request: operations.UpdateServerByUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.Server>,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateServerByUuidRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, z.array(components.Server$inboundSchema)),
    M.jsonErr(
