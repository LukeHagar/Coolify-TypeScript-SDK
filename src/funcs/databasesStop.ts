 ...
 */
export async function databasesStop(
  client: CoolifyCore,
  request: operations.StopDatabaseByUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.StopDatabaseByUuidResponseBody,
    | errors.BadRequest
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.StopDatabaseByUuidRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/databases/{uuid}/stop")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.StopDatabaseByUuidResponseBody$inboundSchema),
    M.jsonErr(
